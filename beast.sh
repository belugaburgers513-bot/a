...
action_connect_server() {
  ...
  echo ""
  echo "=========================================================="
  echo " Run this ONE command on the server you want to connect."
  echo " Your hub's IP is already filled in — nothing to type there."
  echo "=========================================================="
  echo ""
  echo "curl -fsSL ${JOIN_URL} | sudo bash -s -- ${pub_ip} ${name}"
  echo ""
  echo "=========================================================="
  echo " It registers itself with this hub automatically over SSH,"
  echo " sets up its own tunnel + watchdog + resource guard, and"
  echo " you're done. Come back here after and use 'Add port forward'"
  echo " with server name '${name}'."
  echo ""
  echo " Note: that server needs to be able to SSH into THIS hub as"
  echo " root (key-based auth recommended, or it'll prompt for a"
  echo " password when you run the command)."
  echo "=========================================================="
}
